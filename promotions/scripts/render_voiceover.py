...
def render_espeak(text, out_path, voice='en', rate=175):
    if not shutil.which('espeak'):
        raise SystemExit('eSpeak not found in PATH. Install eSpeak or use --provider piper.')

    out_path = Path(out_path)
    needs_convert = out_path.suffix.lower() != '.wav'
    wav_path = out_path if not needs_convert else out_path.with_suffix('.wav')

    cmd = ['espeak', '-v', voice, '-s', str(rate), '-w', str(wav_path), text]
    subprocess.run(cmd, check=True)
...
def main():
    ...
    parser.add_argument('--provider', default='piper', choices=['piper', 'espeak', 'elevenlabs'])
